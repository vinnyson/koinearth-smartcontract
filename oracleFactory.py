...
#            sigR = sp.TSet(t = sp.TBytes),sigV = sp.TSet(t = sp.TNat) -> not needed
...
class Oracle(sp.Contract):
    ...
    def issueCert(self, params):
        ...
        _hash = params._hash
        _signerPublicKey = params.signerPublicKey
        _sigS = params.sigS
        _state = params.state
        _assetType = params.assetType
        _url = params.url
        _publicSignerHash = params.publicSignerHash
        ...
        sp.if _status == 1:
            ...
            self.data.tokenData[_tokenId][_hash].sigS.add(_sigS)
            self.data.tokenAuthSings[_tokenId][_hash] = sp.map({_signerPublicKey : True})             
        sp.else:
            sp.if self.data.tokenAuthSings.contains(_tokenId) & self.data.tokenAuthSings[_tokenId].contains(_hash) & self.data.tokenAuthSings[_tokenId][_hash].contains(_signerPublicKey):
                sp.verify(self.data.tokenAuthSings[_tokenId][_hash][_signerPublicKey] == False)
            self.data.tokenData[_tokenId] = sp.map({_hash: sp.record(oracleContract = sp.self_address, groupId = self.data.groupId, to = _to, toAlias = _toAlias, assetType = _assetType, state = _state, _hash = _hash, issueDateTime = sp.now, url = _url, authoritiesAlias = sp.set([self.data.signerAddressAlias[_signerPublicKey]]), authorities = sp.set([_signerPublicKey]), sigS = sp.set([_sigS]))})
    ...
    def insertWhitelistedAddress(self, params):
        ...
        sp.verify(sp.check_signature(params.publicSignerHash,params.signature,_hash),"verify hash: Invalid signature")
    ...
    # def checkTokenAuth(self, params):
    #     sp.if self.data.tokenAuthSings.contains(params.tokenId) & self.data.tokenAuthSings[params.tokenId].contains(params.)

class OracleFactory(sp.Contract):
    def __init__(self, NFTAddress, factoryAdmin):
        self.oracle = Oracle()
        self.init(OracleList = sp.big_map(tkey = sp.TString ,tvalue = sp.TAddress), lastCalled = sp.address("tz1..."), NFTAddress = NFTAddress, factoryAdmin = factoryAdmin)
    ...
    def _getOracleAddress(self, params):
        ...
        self.data.lastCalled = self.data.OracleList[c]
...
def test():
    ...
    c1 = OracleFactory(NFTAddress = sp.address("KT1..."), factoryAdmin = sp.address("tz1...."))
    scenario += c1
